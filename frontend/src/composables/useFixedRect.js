...
export function useFixedRect(toWorld, domEl) {
  ...
  function getWorldBounds() {
    ...
    const w1 = toWorld(svgRect.x1 + r.left, svgRect.y1 + r.top)
    const w2 = toWorld(svgRect.x2 + r.left, svgRect.y2 + r.top)
    return {
  ...
  function onMouseDown(e) {
    ...
    if (!hasRect.value) {
      drawing = true; isDrawing.value = true
  ...
  function onMouseMove(e) {
    if (drawing) { svgRect.x2 = e.offsetX; svgRect.y2 = e.offsetY }
    else if (dragging) {
      const dx = e.offsetX - dragStart.x, dy = e.offsetY - dragStart.y
      svgRect.x1 = dragStart.rx1 + dx; svgRect.y1 = dragStart.ry1 + dy
      svgRect.x2 = dragStart.rx2 + dx; svgRect.y2 = dragStart.ry2 + dy
    }
  }
  ...
  function onMouseUp(e) {
    let bounds = null
    if (drawing) {
      drawing = false; isDrawing.value = false
      svgRect.x2 = e.offsetX; svgRect.y2 = e.offsetY
      if (Math.abs(svgRect.x2 - svgRect.x1) > 3 && Math.abs(svgRect.y2 - svgRect.y1) > 3) {
        hasRect.value = true; bounds = getWorldBounds()
      }
    } else if (dragging) {
      dragging = false; bounds = getWorldBounds()
    }
    return bounds
  }

  function reset() { hasRect.value = false; isDrawing.value = false; dragging = false; drawing = false }
